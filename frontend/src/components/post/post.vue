  ...
  <v-card>
    <v-card-title class="pa-0 post-header">
      <v-row>
        ...
        <v-btn icon class="position-absolute mr-3">
          <v-icon>
            mdi-dots-vertical
          </v-icon>
        </v-btn>
...
export default {
  ...
  computed: {
    ...
    fullName(){
      return this._props.post.User.firstName + ' ' + this._props.post.User.lastName
    },
    likes(){
    ...
    likefocus() {
      if (this._props.post.userLikeds) {
        if (this._props.post.userLikeds.length === 0) {
          ...
          if (Object.prototype.hasOwnProperty.call(this._props.post.userLikeds[0], "type")) {
